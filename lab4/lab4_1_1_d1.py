...
def testRbf(k1=1.3):
    ...
    ent = entity(dataArr, labelArr, 200, 0.0001, 10000, ("rbf", k1))
...
class SMO:
    ...
    def smop(self):
        ...
        while iter < self.ent.maxIter and ((alphaPairsChanged > 0) or self.entireSet):
            alphaPairsChanged = 0
            # 遍历所有的值
            if self.entireSet:
    ...
    def __innerL(self, i):
        Ei = self.__calcEk(i)
        if (
            (self.ent.labelMat[i] * Ei < -self.ent.toler)
            and (self.ent.alphas[i] < self.ent.C)
        ) or (
            (self.ent.labelMat[i] * Ei > self.ent.toler) and (self.ent.alphas[i] > 0)
        ):
...
class SVM:
    ...
    def fit(self):
        smo = SMO(self.ent)
        self.b, self.alphas = smo.smop()
        self.w = self.__calculateW(self.alphas, self.b)
    ...
    def __calculateW(alphas, dataArr, labelArr):
...
if __name__ == "__main__":
    dataArr, labelArr = loadDataSet("testSetRBF.txt")
    ent = entity(dataArr, labelArr, 0.6, 0.001, ("rbf", 40))
    b, alphas = SMO(ent).smop()
    print(b, alphas)
    testRbf()
    plot_point("lab4\testSetRBF.txt", alphas, dataArr)
